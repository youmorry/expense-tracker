...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ProblemDetail> handleTypeMismatch(
        ...
        log.warn("Type mismatch: method={}, URI={}, param={}", request.getMethod(), request.getRequestURI(), ex.getName());
    ...
    private void logByStatus(int status, AppException ex, HttpServletRequest request) {
        if (status == 404) {
            log.debug("Resource not found: method={}, URI={}, detail={}",
                    request.getMethod(), request.getRequestURI(), ex.getDetail());
        } else if (status >= 500) {
            log.error("Server error: method={}, URI={}, detail={}",
                    request.getMethod(), request.getRequestURI(), ex.getDetail(), ex);
        } else {
            log.warn("Client error: method={}, URI={}, status={}, detail={}",
                    request.getMethod(), request.getRequestURI(), status, ex.getDetail());
        }
    }
