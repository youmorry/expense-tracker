...
- **撮影後は必ず Read ツールで保存ファイルを開き、意図した内容が写っているかを目視確認する**（撮影ツールが返したプレビュー画像と保存ファイルが一致するとは限らない／チャート未描画などの欠落を見逃さないため）
- 確認の粒度は「セクションが映っているか」ではなく「**検証対象の要素そのものが映っているか**」で判定する。対象が映っていなければ撮り直す
