    ...
    <div className="flex flex-col gap-4">
      ...
      <ul className="divide-border divide-y">
        {data.categories.map((category) => {
          ...
          const amountNumber = Number(category.amount);
          return (
            <li key={category.categoryId} className="flex items-center gap-3 px-4 py-3 text-sm">
              <span className="text-xl" aria-hidden="true">
                {emoji}
              </span>
              <span className="text-foreground flex-1 truncate">{category.categoryName}</span>
              <span className="text-foreground font-medium">{formatAmount(amountNumber)}</span>
              <span className="text-muted-foreground w-12 text-right">
                {PERCENT_FORMATTER.format(category.percentage)}%
              </span>
            </li>
          );
        })}
