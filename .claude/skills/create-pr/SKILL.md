---
name: create-pr
description: >
  Pull Request の作成を依頼されたときに使用する。
  プロジェクトの規約に従って GitHub Pull Request の作成を行う。
argument-hint: "[base-branch]"
---

以下の規約に従って Pull Request を作成してください。
ベースブランチは $0 を使用する。指定がない場合は `main` をデフォルトとする。

## 手順

1. 現在の状態を確認する（並列実行）:
   - `git status` で未コミットの変更を確認
   - `git log --oneline $0..HEAD` で PR に含まれる全コミットを確認
   - `git diff $0...HEAD` で差分の全体像を把握
   - リモートとの同期状態を確認
2. 未コミットの変更がある場合は、コミットするかどうかユーザーに確認する。ユーザーが中断を選んだ場合は PR 作成を中止する
3. UI 変更の有無を確認する:
   - 差分に `frontend/src/**` の `.tsx` / `.css` 変更が含まれるかをチェック
   - 含まれる場合、`frontend/screenshots/` 配下のスクリーンショットを一覧して PR 本文の「## スクリーンショット」プレースホルダにそのパスを列挙する
   - スクリーンショットが未取得の場合は、画面検証を行ったか確認し、必要ならユーザーに取得を促す
4. リモートにプッシュする（`git push -u origin <作業ブランチ名>`）
5. `gh pr create` で PR を作成する
6. PR 作成後、UI 変更が含まれる場合はスクリーンショットのファイルパスを再掲し、「PR 本文にドラッグ&ドロップで添付してください」とユーザーに案内する

## PR の規約

- タイトル・本文は**日本語**で記述する
- タイトルは Conventional Commits のプレフィックスを付ける（例: `feat: 支出一覧APIを追加`、`fix: 日付バリデーションを修正`）
- プレフィックスを含めて70文字以内で簡潔に
- Claude が作成した PR であることを本文に明示する
- `gh pr create` を使用し、本文は HEREDOC で `--body` に渡す

## PR 本文のテンプレート

```
## 概要
<!-- 変更の目的と内容を1〜3行で -->

## 変更内容
<!-- 主な変更点を箇条書きで -->

## 関連 Issue
<!-- Closes #123 -->

## スクリーンショット
<!-- UI 変更がある場合のみ。frontend/screenshots/ 配下のファイルをドラッグ&ドロップで添付 -->

## テスト
<!-- テスト方法・確認事項 -->

---
🤖 Generated with [Claude Code](https://claude.ai/code)
```

## 注意事項

- PR に含まれる**すべてのコミット**を分析してタイトルと本文を作成する（最新コミットだけを見ない）
- ブランチ名や コミットメッセージから関連 Issue 番号を特定し、`Closes #<番号>` で自動クローズを設定する。特定できない場合はセクションを省略する
- ブランチ命名が CLAUDE.md の Conventions に定義された規約に従っているか確認する
- `main` ブランチから直接 PR を作成しようとしている場合は警告する
- プッシュおよび PR 作成はユーザーの承認なしで実行してよい
