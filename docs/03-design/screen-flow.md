...
| 2 | 支出一覧 | `/transactions` | 必要 | メイン画面。支出の一覧表示・登録・編集・削除 |
| 3 | 支出分析 | `/analytics` | 必要 | カテゴリ別割合・need/want 比率の表示 |
| 4 | 設定 | `/settings` | 必要 | 通貨変更・アカウント管理 |
...
flowchart TD
    ...
    AuthCheck -- Yes --> Transactions[支出一覧画面]

    Login -- Google認証成功 --> Transactions
...
- Google 認証成功後、ユーザーが DB に存在しなければ自動作成し、支出一覧画面へ遷移する
- 既存ユーザーの場合も支出一覧画面へ遷移する

> **通貨の初期設定について**
> 新規ユーザー作成時にブラウザの `Accept-Language` ヘッダーから通貨コードを自動設定する（詳細は auth-design.md を参照）。
> 通貨選択の専用画面は設けず、変更が必要な場合は設定画面から変更する。

---

### 2. 支出一覧画面（`/transactions`）
...
**支出一覧**
- 日付ごとにグループ化して表示する（日付の新しい順）
- 同じ日付内では登録が新しいもの順（created_at DESC）で表示する
...
sequenceDiagram
    ...
    alt トークンなし or 期限切れ
        ...
        Note over BE: 新規ユーザーの場合、locale から通貨を自動設定
        BE->>FE: JWT + ユーザー情報
    end
...
| ログイン画面 | Google 認証成功 | 支出一覧画面 | リダイレクト |
...
初回ログイン時の通貨選択画面は設けず、ブラウザの `Accept-Language` ヘッダーから通貨を自動設定する:

- 大多数のユーザーは locale から推定した通貨でそのまま利用できるため、専用画面は不要なステップになる
- 認証後すぐにメイン画面を利用できることで、初回体験がスムーズになる
- 通貨を変更したいユーザーは設定画面からいつでも変更可能
